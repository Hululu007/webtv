...
到 [Releases](https://github.com/motao123/webtv/releases) 下载最新版本（当前 v5.5.14 - 安全增强版），按你的设备类型选：
...
- 新增 `network_security_config.xml`：影视源（直播流、点播流）继续允许 `http` 明文通信，但配置源、更新源、远端 JAR 等关键端点（`cnb.cool`、`github.com`、`*.githubusercontent.com`、`gitee.com`）强制 HTTPS。
