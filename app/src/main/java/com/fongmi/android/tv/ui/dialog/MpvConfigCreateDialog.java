...
public class MpvConfigCreateDialog extends BaseAlertDialog {
    ...
    protected void initView() {
        binding.textOption.setVisibility(Util.isLeanback() ? View.GONE : View.VISIBLE);
        if (MpvConfigStore.TARGET_SCRIPTS.equals(target)) binding.scriptWarning.setVisibility(View.VISIBLE);
        setupTvFocus();
    }
    ...
    private void setupTvFocus() {
        ...
        binding.name.setNextFocusDownId(Util.isLeanback() ? R.id.urlOption : R.id.textOption);
    ...
    public void onStart() {
        ...
        if (Util.isLeanback()) binding.urlOption.post(() -> binding.urlOption.requestFocus());
