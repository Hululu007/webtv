...
final class ExoLoadControlModePolicy {
    ...
    private static long targetDurationMs(ExoTargetBufferPolicy.Decision target) {
        ...
        return target == null || bitrate <= 0 ? -1
                : capacityDurationMs(target.targetBytes(), bitrate);
    }

    /** Mirrors webhtv's ExoPlaybackDiagnostics.capacityDurationMs (pure conversion helper). */
    private static long capacityDurationMs(long capacityBytes, long bitrateBitsPerSecond) {
