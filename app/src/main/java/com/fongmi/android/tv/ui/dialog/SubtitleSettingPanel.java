...
final class SubtitleSettingPanel {
    ...
    private void applySecondaryTrack(SecondaryState state) {
        if (player == null || player.isReleased() || !state.supported()) return;
        if (state.trackId() == SubtitleSetting.SECONDARY_SUBTITLE_OFF) {
            player.setSecondarySubtitleTrack(Track.disabled(C.TRACK_TYPE_TEXT, ""));
            return;
        }
        int id = state.trackId() == SubtitleSetting.SECONDARY_SUBTITLE_AUTO ? getFirstSecondaryTrackId(state.tracks()) : state.trackId();
    ...
    private boolean isMpvEngine() {
        return player != null && !player.isReleased() && player.supportsSecondarySubtitle();
    }
