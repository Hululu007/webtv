...
final class ExternalFontSelector {
    ...
    ExternalFontSelector(Fragment fragment, Listener listener) {
        ...
        this.launcher = fragment.registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() != android.app.Activity.RESULT_OK || result.getData() == null) return;
            importFont(result.getData().getData());
        });
    }
    ...
    private void render() {
        ...
        if (restoreFocus && selectedChip != null) selectedChip.requestFocus();
    }
    ...
    private void importFont(@Nullable Uri uri) {
        if (!fragment.isAdded() || loading || uri == null) return;
        loading = true;
        render();
        ...
        Task.execute(() -> {
            try {
                ExternalFont.Item font = ExternalFont.importFrom(resolveUri(uri));
    ...
    private Uri resolveUri(Uri uri) {
        try {
            String path = FileChooser.getPathFromUri(uri);
            if (path != null && !path.isEmpty() && new File(path).isFile()) return Uri.fromFile(new File(path));
