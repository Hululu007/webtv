...
public final class PlaybackPerformanceDialog extends DialogFragment {
    ...
    private void showHelpDialog() {
        ...
        addHelpIntro(content, getString(R.string.player_performance_help_intro, playerName()));
        String section = "";
    ...
    private MaterialButton closeButton(View.OnClickListener listener) {
        ...
        button.setFocusable(true);
        button.setFocusableInTouchMode(Util.isLeanback());
        button.setBackgroundTintList(ContextCompat.getColorStateList(requireContext(), R.color.transparent));
    ...
    private void apply(int profile) {
        switch (profile) {
            ...
            case PlaybackPerformanceSetting.PROFILE_AUTO -> PlaybackPerformanceSetting.applyAuto();
    ...
    private TabLayout createProfileTabs() {
        ...
        tabs.addOnTabSelectedListener(new TabLayout.OnTabSelectedListener() {
            ...
            public void onTabReselected(TabLayout.Tab tab) {
                if (syncingProfileTabs) return;
                if (profileAt(tab.getPosition()) == PlaybackPerformanceSetting.PROFILE_AUTO) apply(PlaybackPerformanceSetting.PROFILE_AUTO);
            }
        });
    ...
    private void syncProfileTabs(TabLayout tabs) {
        ...
        int[] profiles = selectableProfiles();
        boolean rebuilt = !profileTabsMatch(tabs, profiles);
        if (rebuilt) {
            tabs.removeAllTabs();
            for (int profile : profiles) {
                tabs.addTab(tabs.newTab().setText(profileLabel(profile)).setTag(profile), false);
            }
        }
    ...
    private int[] selectableProfiles() {
        return new int[]{PlaybackPerformanceSetting.PROFILE_AUTO, PlaybackPerformanceSetting.PROFILE_RECOMMENDED, PlaybackPerformanceSetting.PROFILE_COMPATIBLE, PlaybackPerformanceSetting.PROFILE_LIGHTWEIGHT};
    }
    ...
    private boolean profileTabsMatch(TabLayout tabs, int[] profiles) {
        ...
        for (int index = 0; index < profiles.length; index++) {
            ...
            if (tab == null || !Integer.valueOf(profiles[index]).equals(tab.getTag())) return false;
        }
        return true;
    }
    ...
    private int profileAt(int position) {
        int[] profiles = selectableProfiles();
        return position >= 0 && position < profiles.length ? profiles[position] : PlaybackPerformanceSetting.PROFILE_AUTO;
    }

    private int profilePosition(int profile) {
        int[] profiles = selectableProfiles();
        for (int index = 0; index < profiles.length; index++) {
            if (profiles[index] == profile) return index;
        }
        return -1;
    }
    ...
    private CharSequence profileLabel(int profile) {
        return switch (profile) {
            case PlaybackPerformanceSetting.PROFILE_LIGHTWEIGHT -> getString(R.string.player_performance_lightweight);
            case PlaybackPerformanceSetting.PROFILE_COMPATIBLE -> getString(R.string.player_performance_compatible);
            case PlaybackPerformanceSetting.PROFILE_AUTO -> getString(R.string.player_performance_auto);
            default -> getString(R.string.player_performance_recommended);
        };
    }

    private List<PlaybackPerformanceOption> options() {
    ...
    private void refreshRows() {
        ...
        for (PlaybackPerformanceOption option : options()) {
            ...
            addRow(option.id(), option.title(), optionValue(option.id()), optionAction(option.id()));
        }
    }
    ...
    private String optionValue(String id) {
        return switch (id) {
            ...
            case PlaybackPerformanceCatalog.BUFFER_TIME -> String.valueOf(PlayerSetting.getBuffer());
    ...
    private Runnable optionAction(String id) {
        return switch (id) {
            ...
            case PlaybackPerformanceCatalog.IJK_BUFFER -> this::cycleIjkBuffer;
    ...
    private void cycleIjkBuffer() {
        int current = IjkPerformanceSetting.getBufferMb();
        IjkPerformanceSetting.putBufferMb(current >= 15 ? 4 : current >= 8 ? 15 : 8);
        refresh();
    }
    ...
    private String ijkBufferText() {
        ...
        if (configuredBytes > 0) return FileUtil.byteCountToDisplaySize(configuredBytes);
        return PlaybackPerformanceSetting.isAuto(PlayerSetting.IJK) ? "自动 · 4～15MB" : IjkPerformanceSetting.getBufferMb() + "MB";
    }
    ...
    private String mpvRenderText() {
        if (PlayerSetting.getMpvRender() == PlayerSetting.MPV_RENDER_VULKAN) {
            return isMpvVulkanAvailable() ? "Vulkan" : "Vulkan（实际回退 OpenGL）";
        }
